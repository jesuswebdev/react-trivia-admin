...
const loadFirstPageSuccess = (state, action) => {
  ...
  return {
    ...
    currentPageNumber: 1,
    currentPage: page,
    totalItems: action.payload.results_count,
...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    case questionActions.LOAD_FIRST_PAGE_SUCCESS:
      return loadFirstPageSuccess(state, action);
    case questionActions.SET_PAGE_NUMBER:
      return setPageNumber(state, action);
    case questionActions.LOAD_PAGE_SUCCESS:
      return loadPageSuccess(state, action);
    case questionActions.NEXT_PAGE:
      return nextPage(state, action);
    case questionActions.LOAD_STATS_SUCCESS:
      return loadStatsSuccess(state, action);
    case questionActions.LOAD_QUESTION_INFO_SUCCESS:
      return loadQuestionInfoSuccess(state, action);
    default:
      return state;
  }
};
